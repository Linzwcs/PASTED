...
def main():
    ...
    def tokenize_and_align_labels(examples):
        ...
        for i, label in enumerate(examples["label"]):
            ...
            word_ids = np.array(input_ids[i])
            label_idxs = np.where(word_ids == 2)[0]

            aligned_label = np.full(word_ids.shape, -100, dtype=np.int32)
    ...
    trainer = Trainer(
        ...
        compute_metrics=compute_metrics_for_seq,
        tokenizer=tokenizer,
