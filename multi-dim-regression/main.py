...
tasks = ["regression-multi-dimension"]
...
class RegressionTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # print("labels:",labels.cpu().numpy().tolist())
        # print("logits:",logits)
        return (
...
class DataCollatorForTokenRegression(DataCollatorForTokenClassification):
    def torch_call(self, features):
        ...
        if padding_side == "right":
            batch[label_name] = [
                to_list(label)
                + [[self.label_pad_token_id] * 3] * (sequence_length - len(label))
                for label in labels
            ]
        else:
            batch[label_name] = [
    ...
    def __post_init__(self):
        ...
        if self.task_name is not None:
            ...
            if self.task_name not in tasks:
                raise ValueError(
                    "Unknown task, you should pick one in " + ",".join(tasks)
                )
...
def main():
    ...
    def tokenize_and_align_labels(examples):
        ...
        for i, label in enumerate(examples["label"]):
            ...
            aligned_label = np.full((word_ids.shape[0], 3), -100, dtype=np.float64)
            # sent_mask = np.zeros(word_ids.shape)
            # print(aligned_label)
            assert len(aligned_label.shape) == 2

            for idx, sub_label in zip(label_idxs, label):
                aligned_label[idx] = sub_label
                # sent_mask[idx] = 1
            # print(word_ids,aligned_label,label)
            labels.append(aligned_label.tolist())
            # print(aligned_label.tolist())
            # sent_masks.append(sent_mask)
        tokenized_inputs["labels"] = labels
        # tokenized_inputs["sent_masks"] = sent_masks
    ...
    trainer = RegressionTrainer(
        ...
        compute_metrics=compute_metrics_for_moe,
        tokenizer=tokenizer,
